...
- 每个被桥接的附件按 SHA-256 文件名保存到项目的 `.opencode/vision-bridge/attachments/`；本地路径不会发送给模型。
...
| `saveDir` | `string` | 项目内 `.opencode/vision-bridge/attachments/` | 附件保存目录；相对路径以会话项目为基准。 |
...
- 上游限制仍然适用；插件另行限制附件为 25 MiB、PDF 最多渲染 32 页、每页 PNG 4 MiB、每条提示最多四个 PDF。
- 成功描述缓存 15 分钟，最多 128 项；失败和部分 PDF 结果不缓存。
- 每个项目复用一个空的隐藏内部 session；OpenCode Promise 插件接口目前无法在 hook 内安全删除它。
- 已保存的摘要文件不会自动删除。
